...
// // Skeleton loader for cards
// const SkeletonCard: React.FC = () => (
//   <div className="flex-shrink-0 w-80 md:w-1/5 snap-center">
//     <div className="bg-white rounded-2xl shadow-md overflow-hidden h-96 animate-pulse">
//       <div className="h-48 bg-gray-200" />
//       <div className="p-6 space-y-4">
//         <div className="h-6 bg-gray-200 rounded w-3/4 mx-auto" />
...
//     setVisibleCards((prev) => Math.min(prev + 4, therapyImprovements.length));
//   };
...
//         <div className="relative">
//           <motion.div
//             className="flex flex-row flex-wrap justify-center gap-6 snap-x snap-mandatory pb-4"
//             variants={containerVariants}
//             initial="hidden"
//             animate="visible"
//           >
//             {therapyImprovements.slice(0, visibleCards).map((item, index) => {
//               const IconComponent = iconMap[item.icon];
//               const isImageLoaded = loadedImages.has(item.bgImage);
...
//                 <motion.div
//                   key={item.title}
//                   className="flex-shrink-0 w-80 md:w-1/5 snap-center"
//                   variants={cardVariants}
//                   custom={index}
//                   whileHover="hover"
//                   role="button"
...
//                     <div className="bg-white rounded-2xl shadow-md overflow-hidden h-96 group relative">
//                       <div
//                         className="h-48 w-full bg-cover bg-center group-hover:scale-105 transition-transform duration-500 ease-in-out"
//                         style={{ backgroundImage: `url(${item.bgImage})` }}
//                       />
//                       <div className="p-6 text-center">
//                         {IconComponent && (
...
//                         </p>
//                       </div>
//                       <div className="absolute inset-0 bg-[#015F4A] bg-opacity-0 group-hover:bg-opacity-60 transition-all duration-300 flex items-center justify-center opacity-0 group-hover:opacity-100">
//                         <span className="text-white font-medium">Learn More</span>
//                       </div>
//                     </div>
//                   ) : (
...
//                     className="hidden"
//                     onLoad={() => handleImageLoad(item.bgImage)}
//                     onError={() => handleImageLoad(item.bgImage)}
//                   />
//                 </motion.div>
//               );
//             })}
...
//           <div className="absolute inset-y-0 right-0 w-8 bg-gradient-to-l from-[#F9F9F9] to-transparent pointer-events-none md:hidden" />
//         </div>
//         {visibleCards < therapyImprovements.length && (
//           <div className="text-center mt-8">
//             <motion.button
//               className="px-8 py-3 rounded-full bg-teal-600 text-white font-medium text-base shadow-md hover:bg-teal-500 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-teal-600 focus:ring-offset-2"
//               variants={buttonVariants}
...
//               aria-label="Show more therapy improvements"
//             >
//               Show More
//             </motion.button>
//           </div>
...
// Define the TherapyImprovement interface
...
const containerVariants: Variants = {
  ...
  visible: {
    opacity: 1,
    transition: {
      staggerChildren: 0.25, // One-by-one entrance
      delayChildren: 0.5,
    },
  },
};
...
const cardVariants: Variants = {
  hidden: { opacity: 0, x: -100, scale: 0.9 }, // Slide from left
  visible: {
    opacity: 1,
    x: 0,
    scale: 1,
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          <motion.div
            ...
            animate="visible"
          >
            {therapyImprovements.slice(0, 4).map((item, index) => {
              const IconComponent = iconMap[item.icon];
              const isImageLoaded = loadedImages.has(item.bgImage);
                ...
                >
                  {isImageLoaded ? (
                    <div className="bg-white rounded-2xl shadow-md overflow-hidden h-96 group relative">
                      ...
                      <div className="p-6 text-center">
                        {IconComponent && (
                          <IconComponent className="w hacking-8 h-8 text-[#31A382] mx-auto mb-4" aria-hidden="true" />
                        )}
                        <h3 className="text-lg font-semibold text-[#015F4A] mb-3">{item.title}</h3>
                        <p className="text-sm text-gray-600 leading-relaxed line-clamp-3">
                          {item.desc}
